<template>
    <div class="h-screen overflow-hidden">
        <header class="bg-red-600">
            <h1 class="text-2xl text-center text-pink-100 font-bold p-1">StopWatch</h1>
        </header>
        <Alert :show="showError">Timer cannot be zero.</Alert>

        <div class="flex flex-col items-center justify-center h-full -mt-10">
            <div class="text-3xl font-bold dark:text-white text-amber-700 mb-3" :class="{ 'text-5xl': startedTimer }">
            ...
            <div v-if="!startedTimer" class="rounded-md shadow-sm hidden lg:inline-flex" role="group">
                <button @click="changeTime('h')" class="relative inline-flex items-center justify-center p-0.5 mb-2 overflow-hidden text-sm font-medium text-gray-900 rounded-l-full group bg-gradient-to-br from-pink-500 to-orange-400 group-hover:from-pink-500 group-hover:to-orange-400 hover:text-pink-100 dark:text-pink-100 focus:ring-4 focus:outline-none focus:ring-pink-200 dark:focus:ring-pink-800">
                    <span class="relative px-5 py-2.5 transition-all ease-in duration-75 bg-pink-100 dark:bg-gray-900 rounded-l-full group-hover:bg-opacity-0 font-bold">
                        {{ positiveClick ? '+' : '-' }} {{ amountByClick }}
                    </span>
                </button>
                <button @click="changeTime('m')" class="relative inline-flex items-center justify-center p-0.5 mb-2 overflow-hidden text-sm font-medium text-gray-900 rounded-x-xl group bg-gradient-to-br from-green-400 to-blue-600 group-hover:from-green-400 group-hover:to-blue-600 hover:text-pink-100 dark:text-pink-100 focus:ring-4 focus:outline-none focus:ring-green-200 dark:focus:ring-green-800">
                    <span class="relative px-5 py-2.5 transition-all ease-in duration-75 bg-pink-100 dark:bg-gray-900 rounded-md group-hover:bg-opacity-0 font-bold">
                        {{ positiveClick ? '+' : '-' }} {{ amountByClick }}
                    </span>
                </button>
                <button @click="changeTime('s')" class="relative inline-flex items-center justify-center p-0.5 mb-2 overflow-hidden text-sm font-medium text-gray-900 rounded-r-full group bg-gradient-to-br from-purple-500 to-pink-500 group-hover:from-purple-500 group-hover:to-pink-500 hover:text-pink-100 dark:text-pink-100 focus:ring-4 focus:outline-none focus:ring-purple-200 dark:focus:ring-purple-800">
                    <span class="relative px-5 py-2.5 transition-all ease-in duration-75 bg-pink-100 dark:bg-gray-900 rounded-r-full group-hover:bg-opacity-0 font-bold">
                        {{ positiveClick ? '+' : '-' }} {{ amountByClick }}
                    </span>
                </button>
            </div>
